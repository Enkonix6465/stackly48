  ...
  const testimonials = [
    {
      ...
      text: "The wellness programs transformed my lifestyle. I feel more energetic, focused, and happy than ever!"
    },
    {
      name: "Michael Lee",
      position: "Entrepreneur",
      text: "The team is amazing! Their personalized guidance helped me manage stress and improve my health holistically."
    },
    {
      name: "Emma Davis",
      position: "Teacher",
      text: "I highly recommend their services. The mindfulness and nutrition plans made a huge difference in my daily life."
    }
  ];
...
>
  ...
  <div className="absolute inset-0 overflow-hidden">
    {[...Array(6)].map((_, i) => (
      <motion.div
        ...
        style={{ top: `${i * 15}%`, left: `-${i * 50}%`, rotate: i % 2 === 0 ? -15 : 15 }}
        animate={{ x: [0, 100, 0] }}
        transition={{ duration: 12 + i * 2, repeat: Infinity, repeatType: "mirror" }}
      />
    ))}
  </div>
  ...
  <div className="relative z-10 flex flex-col md:flex-row items-center justify-center gap-12 px-6">
    {[1, 2, 3].map((i) => (
      <motion.div
        key={i}
        className={themedClass(
        ...
        initial={{ opacity: 0, x: i % 2 === 0 ? -100 : 100, rotate: i % 2 === 0 ? -8 : 8 }}
  ...
  {[...Array(15)].map((_, i) => (
    <motion.div
      ...
      animate={{ y: ["-10%", "10%"] }}
      transition={{ repeat: Infinity, repeatType: "reverse", duration: Math.random() * 4 + 3, delay: Math.random() * 2 }}
    />
  ))}
</section>
...
>
  ...
  {[...Array(6)].map((_, i) => (
    <motion.div
      ...
      transition={{ repeat: Infinity, duration: 12 + i * 2, repeatType: "mirror", ease: "easeInOut" }}
    />
  ))}
  ...
  <div className="relative z-10 flex flex-col md:flex-row justify-around items-start gap-12">
    {[1, 2, 3].map((i) => (
      <motion.div
        key={i}
        className="relative w-full md:w-1/3 p-6 text-justify"
  ...
  <motion.div className={themedClass(
...
>
  ...
  >
    ...
    >
      <Link
        to="/contact"
        className="px-10 py-4 rounded-full shadow-xl bg-gradient-to-r from-orange-400 to-orange-600 text-white font-semibold transition-all duration-300 hover:shadow-2xl"
      >
        {t('contactCta')}
      </Link>
    </motion.div>
