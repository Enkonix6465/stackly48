...
const translations = {
  en: {
    ...
    description: "Empowering learners worldwide with interactive online courses, expert instructors, and real-world skill development for every career stage.",
  ...
  ar: {
    ...
    description: "تمكين المتعلمين حول العالم بدورات تفاعلية عبر الإنترنت، ومدربين خبراء، وتطوير المهارات العملية لكل مرحلة مهنية.",
  ...
  he: {
    ...
    description: "העצמת לומדים ברחבי העולם עם קורסים אינטראקטיביים, מדריכים מומחים ופיתוח מיומנויות לכל שלב בקריירה.",
...
const Footer = () => {
  ...
  return (
    <footer className={`${bg} ${textMain}`} dir={dir}>
      {/* Main Footer Content */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 min-[768px]:grid-cols-4 gap-16 justify-between">
          ...
          <div className="space-y-4">
            <h3 className="text-[#FF7043] font-semibold text-lg">{t("quickLinks", language)}</h3>
            <ul className="space-y-2">
              <li><Link to="/home1" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("home", language)}</Link></li>
              <li><Link to="/about" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("about", language)}</Link></li>
              <li><Link to="/services" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("services", language)}</Link></li>
              <li><Link to="/blog" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("blog", language)}</Link></li>
              <li><Link to="/contact" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("contact", language)}</Link></li>
            </ul>
          </div>

          {/* Column 3 - Our Services */}
          <div className="space-y-4">
            <h3 className="text-[#FF7043] font-semibold text-lg">{t("ourServices", language)}</h3>
            <ul className="space-y-2">
              <li><Link to="/Nutrition" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("nutrition", language)}</Link></li>
              <li><Link to="/Mindful" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("mindful", language)}</Link></li>
              <li><Link to="/Sleep" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("sleep", language)}</Link></li>
              <li><Link to="/Stress" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("stress", language)}</Link></li>
              <li><Link to="/Holistic" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("holistic", language)}</Link></li>
              <li><Link to="/Wellness" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("wellness", language)}</Link></li>
            </ul>
          </div>

          {/* Column 4 - Get In Touch */}
          <div className="space-y-4">
            <h3 className="text-[#FF7043] font-semibold text-lg">{t("getInTouch", language)}</h3>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            ...
            <div className="flex space-x-6 text-sm">
              <a href="#" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("privacy", language)}</a>
              <a href="#" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("terms", language)}</a>
              <a href="#" className={`${textSub} hover:text-[#FF7043] transition-colors duration-200`}>{t("cookie", language)}</a>
            </div>
          </div>
        </div>
      </div>
